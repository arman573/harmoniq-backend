...
export class CustomerChatService {
  ...
  async handleCustomerChat(
    ...
    const capturedFacts = policy.captureCustomerFacts
      ...
      : [];
    const beautyProfileSummary = this.buildBeautyProfileSummary([
      ...existingFacts,
      ...capturedFacts,
    ]);
  ...
  private buildBeautyProfileSummary(facts: CustomerFact[]) {
    const valuesByType = (typeIncludes: string) =>
      uniqueStrings(
        facts
          .filter((fact) => fact.type.toLowerCase().includes(typeIncludes))
          .map((fact) => fact.value),
      ).slice(0, 5);
    const allValues = facts.map((fact) => fact.value);
    const domainsDetected = inferDomains(allValues);
    ...
    return {
      domainsDetected,
      topConcerns: valuesByType('concern'),
      topPreferences: valuesByType('preference'),
      topSensitivities: uniqueStrings([
        ...valuesByType('sensitivity'),
        ...allValues.filter((value) => value.includes('sensitive')),
      ]).slice(0, 5),
...
function inferDomains(values: string[]) {
  const domains = new Set<string>();

  for (const value of values) {
    if (value.includes('hair') || value.includes('scalp')) {
      domains.add('hair');
    } else if (
